...
export function useDevAnalyzer() {
  ...
  const analyze = useCallback(async (username: string) => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed to analyze'); } finally { setLoading(false); }
  }, []);
  ...
  const analyzeRepo = useCallback(async (repo: string) => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed to analyze repo'); } finally { setLoading(false); }
  }, []);
  ...
  const compareRepos = useCallback(async (repo1: string, repo2: string) => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed to compare repos'); } finally { setLoading(false); }
  }, []);
  ...
  const compareDevs = useCallback(async (dev1: string, dev2: string) => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed to compare devs'); } finally { setLoading(false); }
  }, []);
